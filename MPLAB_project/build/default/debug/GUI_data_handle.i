...
volatile _Bool rx_receive_complete = 0;
long int start_point = 0, end_point = 0;
char AT_command[8];
volatile _Bool OBD_reset = 0;
...
void command_service(){
      if (rx_receive_complete == 1) {
        ...
        if (!(strncmp(AT_command, "AT+BAUD=", 8)))
        {

            OBD_event = GUI_baud_rate_change_event;

        }
        if (!(strncmp(AT_command, "AT+CANB=", 8)))
        {
        ...
        if (!(strncmp(AT_command, "AT+REST=", 8)))
        {
            OBD_event = BMS_software_rst_event;
        }

         if (!(strncmp(AT_command, "AT+BOOT=", 8)))
        {
            OBD_event = BMS_bootloader_event;

        }

        if (!(strncmp(AT_command, "AT+CONF=", 8)))
        {
